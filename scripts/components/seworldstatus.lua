...
function seworldstatus:changelist()
	...
	local low_mult = GetModConfigData("SeasonMultiplierLow", "DynamicEconomy")
	local high_mult = GetModConfigData("SeasonMultiplierHigh", "DynamicEconomy")
	local max_price = GetModConfigData("MaxPrice", "DynamicEconomy")

	TUNING.seseasonfood = curseason == "spring" and low_mult or curseason == "winter" and high_mult or 1
	TUNING.seseasoncloth = curseason == "summer" and low_mult or curseason == "winter" and high_mult or 1
	TUNING.seseasonsmithing = curseason == "winter" and low_mult or curseason == "summer" and high_mult or 1
	TUNING.seseasonresource = curseason == "autumn" and low_mult or curseason == "summer" and high_mult or 1
	...
	for k,v in pairs(selist_food) do
		...
		if TUNING.selist_food[k].price > max_price then TUNING.selist_food[k].price = max_price end
		self.repeativecheck = {}
	end
	for k,v in pairs(selist_cloth) do
		...
		if TUNING.selist_cloth[k].price > max_price then TUNING.selist_cloth[k].price = max_price end
		self.repeativecheck = {}
	end
	for k,v in pairs(selist_smithing) do
		...
		if TUNING.selist_smithing[k].price > max_price then TUNING.selist_smithing[k].price = max_price end
		self.repeativecheck = {}
	end
	for k,v in pairs(selist_resource) do
		...
		if TUNING.selist_resource[k].price > max_price then TUNING.selist_resource[k].price = max_price end
		self.repeativecheck = {}
	end
