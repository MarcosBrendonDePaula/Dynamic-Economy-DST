...
local seworldstatus = Class(function(self, inst)
    ...
    self.playerdata = {}
    self.repeativecheck = {}
    self.availableCapital = GetModConfigData("ServerCapital") or 1000000
    self.totalCapital = self.availableCapital
...
-- Nova função para gerenciar o capital do servidor
function seworldstatus:GetInitialCoins()
    local initialCoins = GetModConfigData("InitialCoins") or 200
    if self.availableCapital >= initialCoins then
        self.availableCapital = self.availableCapital - initialCoins
        return initialCoins
    else
        local coins = self.availableCapital
        self.availableCapital = 0
        return coins
    end
end

-- Nova função para retornar moedas ao pool
function seworldstatus:ReturnCoins(amount)
    self.availableCapital = self.availableCapital + amount
    if self.availableCapital > self.totalCapital then
        self.availableCapital = self.totalCapital
    end
end

-- Nova função para obter o ranking de riqueza
function seworldstatus:GetBalanceTop(limit)
    local players = {}
    local totalCoins = self.totalCapital - self.availableCapital
    ...
    local result = {}
    for i = 1, math.min(limit or 10, #players) do
        table.insert(result, players[i])
    end

    return result, totalCoins, self.availableCapital
end
...
function seworldstatus:SumOfRecipePrices(name)
    ...
    for kre,vre in pairs(self.repeativecheck) do
...
function seworldstatus:OnSave()
    local data = {
        ...
        availableCapital = self.availableCapital,
        totalCapital = self.totalCapital,
    }
    return data
end
...
function seworldstatus:OnLoad(data)
    ...
    self.availableCapital = data.availableCapital or GetModConfigData("ServerCapital")
    self.totalCapital = data.totalCapital or self.availableCapital
end
